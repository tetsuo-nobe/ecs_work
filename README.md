...
1. スタックの作成が完了するまで待ちます。(約 5分～10分ほどかかります。)

### 1. コンテナイメージの構築

1. AWS マネジメントコンソールで CloudShell を起動します。
  - 下図の赤枠部分のアイコンをクリックします。

![](images/cloudshell.png)

1. CloudShell のターミナル で下記のコマンドを実行し、Docker がインストールされていることを確認します。
...
1. 下記のコマンドでサンプルのアプリケーションのコードを確認します。(Python で Flask という Webアプリケーションフレームワークを使用しています。）
...
1. Dockerfile を確認します。
   - この Dockerfile を使用してコンテナのイメージをビルドします。(シンプルにするためコンテナとして最適化していません）
...
1. **ヘルスチェックの猶予期間** に `120` と入力します。（これにより ELB のヘルスチェックによるタスクの強制停止の影響を回避します。）

1. ページを少し下にスクロールし、**ネットワーキング** セクションを展開表示します。
...
1. **リスナー** で **既存のリスナーを使用** を選択し、**HTTP:80** を選択します。
